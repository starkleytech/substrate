...
use super::Call as BalancesCall;
...
pub fn force_transfer<T>(node: &Node<T>)
    where
        T: ChainInfo,
        T::Runtime: pallet_sudo::Config + super::Config,
        ...
        <T::Runtime as super::Config>::Balance: From<u128>,
...
{
    type Balances<T> = super::Module<T>;
    let (alice, bob) = (
        ...
        AccountId32: std::borrow::Borrow<<T::Runtime as frame_system::Config>::AccountId>,
{
    ...
    let balance = bob_balance - (<T::Runtime as super::Config>::ExistentialDeposit::get() / 2u128.into());
